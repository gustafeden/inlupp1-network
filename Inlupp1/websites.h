...
void selectionPage() {
	if (!usernamecheck && !passwordcheck) {
		...
		if (showtemps) {
			fullpage = fullpage + "<div class='d-flex justify-content-center'><div class='p-2'>Temp recieved from other device :</div><div class='p-2'>  " + radiomodule->getTemps() + "</div></div>";
		}
